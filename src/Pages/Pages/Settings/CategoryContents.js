...
import {getAllCategories, getCategories} from '../../../Service/CategoryService';
import {
    useNavigate
...
const CategoryContents = ({sendCategoryToParent, all_categories_included}) => {
    ...
    //useEffect(() => console.log(search_results), [search_results]);
    ...
    const onCategoryClick = (e, category_id) => {
        ...
        let params = "";
        if(all_categories_included === true) {
            params = {mode: "master"}
        }
        getCategories(category_id, params).then(res => {
            if(res.status == 200){
                if(res.data) 
                setServiceCategoriesChildrenList(res.data);
            } else {
                Swal.fire({
                    icon: 'error',
                    title: 'Error!',
    ...
    useEffect(() => {
        if(userInfo.access){
            if(userInfo.access.services){
                ...
                getCategories(null, params).then(res => {
                    if(res.status == 200){
                            ...
                            <div className="card-body">
                                ...
                                <div style={{display:"flex", justifyContent:"space-between"}}>
                                    ...
                                    <div style={{display:"flex", justifyContent:"flex-end", columnGap:"10px"}}>
                                        <input placeholder="Search for a service item" onChange={(e) => setKeyword(e.target.value)} style={{borderRadius:"8px", padding:"0 10px", fontSize:"14px", width:"220px"}}/>
                                ...
                                {service_categories_list && Object.keys(service_categories_list).length > 0 && !keyword &&
                                    <div className='row' style={{marginTop:"25px"}}>
                                        ...
                                        <div className='col-md-3'>
                                            ...
                                            {service_categories_list.map(item => {
                                                ...
                                                <li style={{cursor:"pointer", marginBottom:"10px",marginTop:"10px"}} onClick={(e) => onCategoryClick(e, item.id)} >
                                                        <a className="catalog-parent-link" style={{ display:"flex", columnGap:"10px", alignItems:"center", fontSize:"15px"}}>
                                        ...
                                        <div className='col-md-9' style={{paddingLeft:"30px"}}>
                                            ...
                                            {service_categories_children_list && service_categories_children_list.length > 0 &&
                                                <div style={{display:"flex", columnGap:"20px", flexWrap:"wrap"}}>
                                                {service_categories_children_list.map(item => {
                                                    return(
                                                    <div style={{cursor:"pointer", marginBottom:"10px",marginTop:"10px"}} onClick={(e) => setCategory(e, item)}>
