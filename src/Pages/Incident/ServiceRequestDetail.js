...
import {isObjectEmpty, isValidEmail} from '../../Utils/Utils';
...
import { getRequest, forwardRequest, updateRequest, resolveRequest } from '../../Service/ServiceRequestService';
...
const ServiceRequestDetail = () => {
    ...
    const [ticket_fields, setTicketFields] = useState({});
    const [boolean_ticket_fields, setBooleanTicketFields] = useState({});
    ...
    //useEffect(() => console.log(boolean_ticket_fields), [boolean_ticket_fields]);
    ...
    useEffect(() => {
        if(refresh) {
            ...
            getRequest(request_id).then(res => {
                if(res.status == 200){
                    if(res.data.ticket_fields){
                        let parsed_fields = JSON.parse(res.data.ticket_fields);
                        if(parsed_fields.ticket_attachment) delete parsed_fields.ticket_attachment
                        

                        Object.keys(parsed_fields).forEach(item => {
                            if(typeof parsed_fields[item] == 'boolean'){
                                setBooleanTicketFields({...boolean_ticket_fields, [item]: parsed_fields[item]})
                                delete parsed_fields[item];
                            }
                        })

                        setTicketFields(parsed_fields);
                        delete res.data.ticket_fields;
                    }
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        text: "Failed to get request data. Please try again or contact support"
                    }).then(() => navigate('/service-requests'));
                }
            });

            getSLAMonitoring(request_id).then(res => {
                if(res.status == 200){
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        text: "Failed to get service request data. Please try again or contact support"
                    }).then(() => navigate('/service-requests'))
                }
            }).catch(err => {
                ...
                getRequest(request_id).then(res => {
                    if(res.status == 200){
                        if(res.data.ticket_fields){
                            ...
                            else if(userInfo.role_name == "Agent"){
                                if(userInfo.email !== res.data.forward_to_email){
        ...
        if(field_name == "status" && value == RESOLVED_STATUS){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                  ...
                  resolveRequest(request_id, {agent_remark: textarea_value}).then(res => {
                    if(res.status == 200) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            text: "Request has been resolved and notified to user!"
                         }).then(() =>{
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error!',
                            text: "Failed to get resolve request. Please try again or contact support"
                        });
                    }
                  })
                }
            ...
            updateRequest(request_id, request_data).then(res => {
    ...
    const onChangeEmail = (e) => {
        //setSelectedHelper({recipient_email:e.target.value});
    ...
    const onChangeAgent = (e) => {
        if (e.target.value.length) {
            ...
            agent_timer_id = setTimeout(() => getAllAgents({search:e.target.value, filter:`group_id:${request_data.group_id}`}).then((res) => {
        ...
        updateRequest(request_id, {
            ...
            if(stats == 200){
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: "A request has been updated!"
                    }).then( () => navigate('/service-requests'));
    ...
    const onForwardTicket = (e) => {
        ...
        forwardRequest(request_id, selected_helper).then(res => {
            if(res.status == 200){
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
    ...
    const onImageChange = (e) => {
        ...
        if (file) {
            ...
            if(file.size <=1000000){
                ...
                formData.append("ticket_id", request_data.ticket_id);
                    ...
                    <div className="card shadow mb-4">
                        ...
                        <div className="row mb-2">
                            <div className="col-sm-6">
                                <div style={{display:"flex", alignItems:"center", cursor: "pointer", marginBottom:"10px", fontSize:"16px", color:"#FAA819"}} onClick={() => navigate('/service-requests')}>
                                            <div><span class="material-icons" style={{fontSize:"18px", marginTop:"5px"}}>arrow_back</span></div>
                                            <div>&nbsp; Back to Service Requests List</div>
                                </div>
                                <div style={{display: "flex", columnGap: "20px"}}>
                                    <div className="ticket-title fw-500 black" style={{display: "block"}}>
                                            Ticket #{request_data.ticket_number}
                                    </div>
                                    <div>
                                        <div>
                                            {renderStatusBadge(request_data.status_name)}
                                        </div>
                                    </div>       
                                </div>
                                <div>
                                        <div style={{fontSize: "16px", display: "block", marginTop:"15px"}}>Service Item: &nbsp;<span className="bold black">{request_data.category_name}</span></div>
                                        <div style={{fontSize: "16px", display: "block"}}>Requested Date:  &nbsp;<span className="bold black">{moment(request_data.requested_date).format('ll')}</span></div>
                                        <div style={{fontSize: "16px", display: "block"}}>Requester:  &nbsp;<span className="bold black">{request_data.requester} ({request_data.requester_email})</span></div>
                                        <div style={{fontSize: "16px", display: "block"}}>Requested For:  &nbsp;<span className="bold black">{request_data.requested_for} ({request_data.requested_for_email})</span></div>
                                </div>
                                    ...
                                    <div className='row' style={{marginBottom: "20px", width: "100%"}}>
                                        <div className='col-md-9 col-sm-12'>
                                            <div style={{fontSize: "16px", paddingTop:"10px", width: "95%"}}>
                                                <div className="row">
                                                    <div className={(!boolean_ticket_fields || isObjectEmpty(boolean_ticket_fields))? "col-md-8": "col-md-6"}style={{display:"flex", columnGap: "30px", flexWrap:"wrap"}}>
                                                        {ticket_fields && !isObjectEmpty(ticket_fields) && Object.keys(ticket_fields).map(item => {
                                                            let value = ticket_fields[item];
                                                            if(Array.isArray(ticket_fields[item])){
                                                                value = "";
                                                                let i = 0;
                                                                ticket_fields[item].forEach(stuff => {
                                                                    value += stuff.value;
                                                                    if(i < ticket_fields[item].length - 1) value +=",";
                                                                    i++;
                                                                })
                                                            }
                                                            return(
                                                            <div className="ticket-detail" style={{display:"flex", flexDirection:"column", rowGap:"5px"}}>
                                                                <div className="bold black">{capitalize(item)}:</div>
                                                                <div>{value}</div>
                                                            </div>
                                                            )
                                                        })}
                                                    </div>
                                                    {boolean_ticket_fields && !isObjectEmpty(boolean_ticket_fields) && <div className="col-md-2" style={{display:"flex", rowGap: "10px", flexWrap:"wrap", flexDirection:"column"}}>
                                                        {Object.keys(boolean_ticket_fields).map(item => {
                                                            return(
                                                                <div className="ticket-detail" style={{display:"flex", flexDirection:"column", rowGap:"5px"}}>
                                                                    <div className="bold black">{capitalize(item)}:</div>
                                                                    <div>
                                                                        <InputSwitch checked={boolean_ticket_fields[item]} disabled/>
                                                                    </div>
                                                                </div>
                                                            )
                                                        })}
                                                    </div>}
                                                    {userInfo.role_name !== "Administrator" && request_data.status !== CLOSED_STATUS && <div className='col-md-4' style={{display:"flex", flexDirection:"column", alignItems:"flex-end", rowGap:"5px"}}>
                                                        {userInfo.role_name == "Requester" &&<div style={{display:"flex", color:"#FAA819", alignItems:"center", columnGap:"5px", float:"right", cursor:"pointer"}} onClick={()=> navigate(`/service-form/edit/${request_data.category}/${request_id}`)}>
                                                            <div><FaEdit/></div>
                                                ...
                                                <div style={{marginBottom:"40px", marginTop:"20px"}}>
                                                    ...
                                                    <div style={{float:"right"}}>
                                                        <button class="btn right" style={{padding:"0 !important", fontSize:"14px",background:"#FAA819", color:"white"}} onClick={submitConversation} disabled={(request_data.status === CLOSED_STATUS || new_conversation.length == 0)}>
                                                            Submit
                                                        </button>
                                                    </div>
                                                    <div style={{display: "flex", alignItems:"center", columnGap: "10px", justifyContent: "flex-start", marginTop:"60px"}}>
                                                            ...
                                                            <div style={{display: "flex", alignItems:"center", columnGap: "10px"}}>
                                                                <button class="btn right" style={{padding:"0 !important", fontSize:"14px",background:"#FAA819", color:"white"}} onClick={()=> $('#attachment-request-upload').click()} disabled={request_data.status === CLOSED_STATUS}>
                                                                    Browse Files
                                                                </button>
                                                                <input id="attachment-request-upload" type="file" accept="image/png, image/jpg, image/jpeg, application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, .pdf, video/mp4, message/rfc822" className='d-none' onChange={(e) =>onImageChange(e)}/>
                                                            </div>
                                                    </div>
                                                ...
                                                {conversation_state === "Replies" && request_data.ticket_conversations && request_data.ticket_conversations.length > 0 && request_data.ticket_conversations.map(item => {
                                                        ...
                                                        <li class="box_result row">
                                                            ...
                                                            <>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <NavLink to={`/conversation-content/service-request/${item.conversation_id}`}>Link to email excerpt</NavLink>
                                                                </div>
                                                ...
                                                {conversation_state === "Files" && request_data.ticket_files && request_data.ticket_files.length > 0 && request_data.ticket_files.map(item => {
                                                    return(
                                                        <li class="box_result row">
                                                    ...
                                                    <div className='card-body'>
                                                        <div>
                                                            ...
                                                            <div style={{marginBottom:"20px"}}>
                                                                <div className='black' style={{fontSize:"18px", display:"flex"}}>
                                                                ...
                                                                {enable_edit_urgency === false && <div>{request_data.urgency? renderPriority(request_data.urgency): "Unassigned"}</div>}
                                                                ...
                                                                {enable_edit_urgency === true &&  <select className="form-select" style={{padding:"10px", marginTop:"10px"}} aria-label="Default select example" disabled={userInfo.role_name =="Requester"} onChange={(e) => onChangeData("urgency", e.target.value)}>
                                                                    <option selected={request_data.priority == "urgent"} value="urgent">Urgent</option>
                                                                    <option selected={request_data.priority == "high"} value="high">High</option>
                                                                    <option selected={request_data.priority == "medium"} value="medium">Medium</option>
                                                                    <option selected={request_data.priority == "low"} value="low">Low</option>
                                                                </select>}
                                                            </div>
                                                            <div style={{display:"flex"}}>
                                                                
                                                                <div>
                                                                    <div className='black' style={{fontSize:"0.8em"}}><b>First Response Due:</b></div>
                                                                    <div className='estimation-time black' style={{fontSize:"0.8em", display:"flex", columnGap:"10px"}}>
                                                                    <div>{sla_response ? moment(sla_response).format("DD MMM YYYY HH:mm"): "--"}</div>
                                                                        <div>
                                                                            {request_data.is_overdue_response === false && (sla_monitoring && sla_monitoring.response &&  sla_monitoring.response.sla_comparison_result == "Within SLA")&&<span className= "badge badge-success">Within SLA</span>}
                                                                            {(request_data.is_overdue_response === true || (sla_monitoring && sla_monitoring.response &&  sla_monitoring.response.sla_comparison_result == "Overdue" && !request_data.is_overdue_response))&&  <span className= "badge badge-danger">Overdue</span>}
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            ...
                                                            <div style={{display:"flex", marginBottom:"10px"}}>
                                                               <div>
                                                                    <div className='black' style={{fontSize:"0.8em", marginTop:"10px"}}><b>Till First Response Due:</b></div>
                                                                    <div className='estimation-time black' style={{fontSize:"0.8em", display:"flex", columnGap:"10px"}}>
                                                                    <div>
                                                                        ...
                                                                        {sla_monitoring && sla_monitoring.response && sla_monitoring.response.sla_comparison_result == "Overdue" && !request_data.overdue_response_duration && <div className="bold" style={{color:"#e74a3b"}}>
                                                                            -{till_sla_response_due}
                                                                        </div>}

                                                                        {sla_monitoring && sla_monitoring.response && sla_monitoring.response.sla_comparison_result == "Overdue" && request_data.overdue_response_duration && <div className="bold" style={{color:"#e74a3b"}}>
                                                                            -{request_data.overdue_response_duration} minutes
                                                                        </div>}
                                                                    </div>
                                                            ...
                                                            <div style={{display:"flex"}}>
                                                                <div>
                                                                    <div className='black' style={{fontSize:"0.8em"}}><b>Resolve Due:</b></div>
                                                                    <div className='estimation-time black' style={{fontSize:"0.8em", display:"flex", columnGap:"10px"}}>
                                                                    <div>{sla_resolve ? moment(sla_resolve).format("DD MMM YYYY HH:mm"): "--"}</div>
                                                                            <div>
                                                                                {!request_data.is_overdue_resolve && <span className= "badge badge-success">Within SLA</span>}
                                                                                {request_data.is_overdue_resolve === true && <span className= "badge badge-danger">Overdue</span>}
                                                                            </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div style={{display:"flex"}}>
                                                                <div>
                                                                    <div className='black' style={{fontSize:"0.8em",marginTop:"10px"}}><b>Till Resolve Due Estimation:</b></div>
                                                                    <div className='estimation-time black' style={{fontSize:"0.8em", display:"flex", columnGap:"10px"}}>
                                                                    <div>
                                                            ...
                                                            {(userInfo.role_name == "Agent" || userInfo.role_name == "Agent Supervisor" || userInfo.email == request_data.forward_to_email) && sla_monitoring && sla_monitoring.resolve && <div style={{display:"flex"}}>
                                                    ...
                                                    <div className='card-body'>
                                                        <div className='form-group full-width sd-form-group' style={{fontSize:"16px"}}>
                                                            ...
                                                            <div style={{ border: 'solid 1px #ccc', borderRadius: 4 }}>
                                                                <select className="form-select inputLogin" aria-label="Default select example" value={request_data.status} onChange={(e) => onChangeData("status", e.target.value)} disabled={disableStatusChange(userInfo, request_data)}>
                                                                {statuses && statuses.length > 0 && statuses.map(item => {
                                                                        if(item.status_name == "Closed" && (userInfo.role_name == "Agent" || userInfo.role_name == "Agent Supervisor") && request_data.status !== CLOSED_STATUS) {
                                                                            return null;
                                                                        }
                                                                        else return(<option selected={request_data.status == item.id} value={item.id}>{item.status_name}</option>)
                                                                    }) }
                                                                </select>
                                                            </div>
                                                        <div className='form-group full-width sd-form-group' style={{fontSize:"16px"}}>
                                                            ...
                                                            <div style={{ border: 'solid 1px #ccc', borderRadius: 4 }}>
                                                                <select className="form-select inputLogin" aria-label="Default select example" disabled={userInfo.role_name =="Requester"} onChange={(e) => onChangeData("impact", e.target.value)}>
                                                                    <option selected={request_data.impact == "high"} value="high">High</option>
                                                                    <option selected={request_data.impact == "medium"} value="medium">Medium</option>
                                                                    <option selected={request_data.impact == "low"} value="low">Low</option>
                                                                </select>
                                                            </div>
                                                        </div>
                                                        <div className='form-group full-width sd-form-group' style={{fontSize:"16px"}}>
                                                        ...
                                                        <div className='form-group full-width sd-form-group' style={{fontSize:"16px", }}>
                                                            ...
                                                            */}
                                                            ...
                                                            {request_data.group_id && edit_group == false && <div style={{border:"1px solid rgb(182, 199, 206)", borderRadius:"4px", padding:"10px"}}>{request_data.group_name}</div>}
                                                        ...
                                                        {edit_agent === false && <div className='form-group full-width sd-form-group' style={{fontSize:"16px"}}>
                                                            ...
                                                            <div style={{ border: 'solid 1px #ccc', borderRadius: 4 }}>
                                                                <input className='inputLogin' value={request_data.agent_name} disabled/>
                                                            </div>
                                                        </div>}
                                                        {edit_agent === true && <div className="form-group" style={{marginTop:"10px"}}>
                                                            ...
                                                            <div style={{marginTop:"5px"}}>
                                                                <i>* Pick anyone from {`${request_data.group_name}`}</i>
                                                            </div>
                                ...
                                <>
                                    ...
                                    <div>
                                        ...
                                        <ol class="activity-feed">
                                            {request_data.ticket_logs && request_data.ticket_logs.length > 0 && request_data.ticket_logs.map(item => {
                                ...
                                <>
                                    <div className='row' style={{marginBottom: "20px", border: "1px solid #D0D5DD", borderRadius: "4px", width: "100%"}}>
                                        <div className='col-md-12 col-sm-12'>
                                            ...
                                            {request_data.ticket_attachment && 
                                                <table className="table table-condensed" style={{ marginTop: 16}}>
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <>
                                                            <td>{request_data.ticket_attachment}</td>
                                                            <td>
                                                                    <a href={`${PHOTO_BASE_URL}incident_attachments/${request_data.ticket_attachment}`} className='download-attachment' style={{display: "flex", alignItems: "center", columnGap: "10px", cursor:"pointer"}} download>
                                                                        <img src={chrome_tote}/>
                                                                        <div>Download</div>
                                                                    </a>
                                                            </td>
